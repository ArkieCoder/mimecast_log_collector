...
class Mimecast:
  ...
  def create_signature(self, data_to_sign: str, secret_key: str, encoding="utf-8"):
    ...
    digest = hmac.new(
      secret_key, data_to_sign, digestmod=hashlib.sha1
    ).digest()  # still bytes
  ...
  def get_base_url(self, email_address):
    ...
    # Send request to API
    log.debug(
      "Sending request to https://api.mimecast.com/api/discover-authentication with request ID: %s"
      % (request_id)
    )
    try:
      r = requests.post(
        url="https://api.mimecast.com/api/login/discover-authentication",
    ...
    if "region" in resp_data["data"][0]:
      ...
      log.error(
        "No region information returned from API, please check the supplied email address in configuration.py. Cannot continue"
      )
      quit()
