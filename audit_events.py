...
def init_directory(event_category):
  if not os.path.exists(
    configuration.logging_details["CHK_POINT_DIR"] + str(event_category)
  ):
    os.makedirs(
      configuration.logging_details["CHK_POINT_DIR"] + str(event_category)
    )
...
def get_audit_events(base_url, access_key, secret_key):
  post_body = dict()
  post_body["data"] = [
    {"startDateTime": get_old_date(), "endDateTime": get_current_date()}
  ]
